...
def load_model():
  mwd_model=tf.keras.models.load_model('predict_weather(classification).h5')
  return mwd_model
mwd_model=load_model()
st.write("""
...
import numpy as np
def import_and_predict(image_data,mwd_model):
    size=(64,64)
    image=ImageOps.fit(image_data,size,Image.ANTIALIAS)
    img=np.asarray(image)
    img_reshape=img[np.newaxis,...]
    prediction=mwd_model.predict(img_reshape)
    return prediction
if file is None:
    ...
    prediction=import_and_predict(image,mwd_model)
